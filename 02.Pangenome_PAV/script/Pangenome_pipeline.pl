...
foreach my $sam (sort keys %genomes) {
	...
	print $SH01a "ln -s $genome $db_dir/$sam.genome.fa\n";
...
foreach my $sam (sort keys %genomes) {
	...
	foreach my $fa (@fa) {
		...
		print $SH02 "$bincf{mummer}/nucmer --maxmatch -c 50 -l 40  -t 4 $refgenome $fa  -p $output_nucmer\n";
		close $SH02;
	}
}
#### 过滤
...
foreach my $sam (sort keys %genomes) {
	...
	print $SH04 "perl $bincf{bindir}/script/step04.merge_filt.pl $Mummer_dir/$sam $refgenome $genomes{$sam} $delta_onebyone_merge\n";
...
foreach my $sam (sort keys %genomes) {
	...
	print $SH07 "#$bincf{java} -Xmx4g -Djava.io.tmpdir=$gatk_dir/$sam/temp/ -XX:MaxPermSize=512m -XX:-UseGCOverheadLimit -jar $bincf{gatk}/GenomeAnalysisTK.jar -T HaplotypeCaller -R $self_genome -I $bam_rmdup_self --emitRefConfidence GVCF --variant_index_type LINEAR --variant_index_parameter 128000 -o $gvcf_pri -nct 2 \n";
...
foreach my $sam (sort keys %genomes) {
	...
	open my $SH081,">$shell_dir/S08.CNV/$sam/step081.depth.ref.sh" or die $!;
	print $SH081 "$bincf{samtools}/samtools depth -Q 20 -a $bam_rmdup_ref |gzip >$depth_ref \n";
...
foreach my $sam (sort keys %genomes) {
	...
	close $SH15;
	open my $SH16,">$shell_dir/S10.PAV/$sam/step16.$sam.map_unmap.sh" or die $!;
	my $filt1="$pav_dir2/$sam/$sam.filt.sam.gz";
	my $filt2="$pav_dir2/$sam/$sam.ref.filt.sam.gz";
	my $output1="$pav_dir2/$sam/$sam.align.gz";
	my $output2="$pav_dir2/$sam/$sam.ref.align.gz";
	my $map1="$pav_dir2/$sam/$sam.map";
	my $map2="$pav_dir2/$sam/$sam.ref.map";
	my $unmap1="$pav_dir2/$sam/$sam.unmap";
	my $unmap2="$pav_dir2/$sam/$sam.ref.unmap";

	print $SH16 "perl $bincf{bindir}/script/step22.filt.pl $fq  $sortbam $filt1 \n";
	print $SH16 "perl $bincf{bindir}/script/step22.filt.pl $ref_fq  $sort_refbam $filt2 \n";
	print $SH16 "perl $bincf{bindir}/script/step22.match.pl $fq  $filt1 $output1 \n";
	print $SH16 "perl $bincf{bindir}/script/step22.match.pl $ref_fq  $filt2 $output2 \n";
	print $SH16 "perl $bincf{bindir}/script/step23.map_scale.pl $sam_genome $output1  $map1 \n";
	print $SH16 "perl $bincf{bindir}/script/step23.map_scale.pl $refgenome $output2  $map2 \n";
	print $SH16 "perl $bincf{bindir}/script/step24.unmap_scale.pl $sam_genome $map1 $unmap1 \n";
	print $SH16 "perl $bincf{bindir}/script/step24.unmap_scale.pl $refgenome  $map2 $unmap2 \n";
	print $SH16 "perl $bincf{bindir}/script/step25.unmap_merge.pl $unmap1 $unmap1.merge\n";
	print $SH16 "perl $bincf{bindir}/script/step25.unmap_merge.pl $unmap2 $unmap2.merge\n";
	close $SH16;

}
	open my $SH17,">$shell_dir/S10.PAV/step17.core.sh" or die $!;
	my $option="$refgenome.fai ";
	foreach my $sam (sort keys %genomes) {
		$option.=" $pav_dir2/$sam/$sam.ref.map "
	}
	my $depthnum=scalar(keys %genomes);
	&creatdir("$pav_dir2/Pangenome");
	print $SH17 "perl $bincf{bindir}/script/step26.core_genome.pl $option $pav_dir2/Pangenome/SB\n";
	print $SH17 "perl $bincf{bindir}/script/step27.core_genome_reg.pl $pav_dir2/Pangenome/SB.CG.depth $depthnum $pav_dir2/Pangenome/SB.CG.cover\n";
	close $SH17;
