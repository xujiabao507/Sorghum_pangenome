...
foreach my $chr (sort keys %genest) {
	...
	foreach my $bins (sort {$a<=>$b} keys %{$chrbins{$chr}}) {
		my @locs=sort keys %{$chrbins{$chr}{$bins}};
		my $num_loc=scalar(@locs);
		print "$chr\t$bins\t$num_loc\n";
	}
	$scale{$chr}{$bin}="$loc[0]\t$loc[-1]";
}
...
while (<$DF>) {
	...
	if(exists $genelocdb{$chr}{$pos}){
		foreach my $genet (sort keys %{$genelocdb{$chr}{$pos}}) {
			...
			print $OUT "$chr\t$pos\t$depth\t$genet\t$genescale{$chr}{$genet}\n";
